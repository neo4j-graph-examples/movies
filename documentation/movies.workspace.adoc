...
[TIP]
====
You can click on the query above to populate it in the Cypher Editor in the Query tab.
====
...
Labels is a name or identifier of a node or a relationship. 
...
You can assign the node label to a variable by prefixing the syntax with the variable name. Like `(p:Person)` means `p` variable denoted `Person` labeled nodes.
